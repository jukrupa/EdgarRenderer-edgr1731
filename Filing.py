                    ...
                    if fact.concept is None:
                        ...
                        if not self.validatedForEFM:
                            self.modelXbrl.error("EFM.6.07.19", # use standard Arelle message for this
                                _("[cp-0719-No-Tuple-Element] You provided an extension concept which is a tuple, $(concept).  Please remove tuples and check your submission."),
        ...
        for context in self.modelXbrl.contexts.values():
            if context.scenario is not None and not self.validatedForEFM:
                ...
                self.modelXbrl.error("EFM.6.05.05", # use standard arelle message
                                _("[cp-0505-Segment-Child-Not-Explicit-Member] There must be no segments with non-explicitDimension content, but $(count) was(were) found: $(context)."),
        ...
        while len(commandTextList) > 0:
            ...
            if token0Lower in {'row', 'column'}:
                ...
                self.modelXbrl.error("EFM.6.26.04.embeddingCmdMalformedDirectionToken",
                                     _("[rq-2604-Embedding-Command-Malformed-Direction-Token] In ''$(linkroleName)'', the embedded report created by the fact $(fact) with the context $(contextID), the token $(token), at position $(position) in the list of tokens, is malformed. Each iterator can only start with 'row' or 'column'."),
            ...
            if token2Lower in {'compact', 'nodisplay'}:
                ...
                self.modelXbrl.error("EFM.6.26.04.embeddingCmdMalformedStyleToken",
                                     _("[rq-2604-Embedding-Command-Malformed-Style-Token] In ''$(linkroleName)'', the embedded report created by the fact $(fact), with the context $(contextID), the style keyword $(style) is not one of 'compact' or 'nodisplay'."),
                                     modelObject=fact, linkrole=linkroleUri, fact=fact.qname, contextID=fact.contextID,
                                     linkroleDefinition=self.modelXbrl.roleTypeDefinition(linkroleUri), linkroleName=self.modelXbrl.roleTypeName(linkroleUri), 
                                     style=token2)
                return False
            ...
            if invalidTokens:
                ...
                self.modelXbrl.error("EFM.6.26.04.embeddingCmdMalformedMember",
                                     _("[rq-2604-Embedding-Command-Malformed-Member] In ''$(linkroleName)'', the embedded report created by the fact $(fact) with the context $(contextID), the keyword(s) $(tokenlist) is not '*', a valid member qname or list of valid member qnames."),
                                     modelObject=fact, linkrole=linkroleUri, fact=fact.qname, contextID=fact.contextID,
                                     linkroleDefinition=self.modelXbrl.roleTypeDefinition(linkroleUri), linkroleName=self.modelXbrl.roleTypeName(linkroleUri), 
