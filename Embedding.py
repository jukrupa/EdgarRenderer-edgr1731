                ...
                for axisQname, (ignore, presentationGroupOrderForAxis) in self.cube.axisAndMemberOrderDict.items():
                    if presentationGroupOrderForAxis == Utils.minNumber and axisQname not in self.setOfGivenAxes:
                        ...
                        self.filing.modelXbrl.warning("EFM.6.26.06",
                                _('[rq-2606-Axis-Has-No-Order] In ''$(linkroleName)'', the embedded report created by the fact $(fact) with context $(contextID), the axis $(axis) was not given an order in the presentation group $(linkroleDefinition). The axes are being sorted by their labels.'),
    ...
    def processCommandBuildgetMemberPositionsOnAxisDictOfDicts(self):
        if self.cube.isElements and self.pseudoAxis == 'primary' and self.rowOrColumn == 'column':
            ...
            self.filing.modelXbrl.warning("EFM.6.26.09",
                    _('[rq-2609-Primary-Axis-On-Rows] In ''$(linkroleName)'' the embedded report created by the fact $(fact) with context $(contextID) contains an iterator "column primary" even though the definition text of $(linkroleDefinition) contains the "{Elements}" qualifier. The primary axis will remain on the rows.'),
