        ...
        if not cubeNumberIsANumber or '' in (hyphen, secondHyphen, cubeNumber, cubeType, cubeName):
            if not self.filing.validatedForEFM:
                self.filing.modelXbrl.error("EFM.6.07.12", # use identical EFM message & parameters as Arelle Filing validation
                    _("[rq-0712-Role-Definition-Mismatch]  The definition '$(definition)' of role $(roleType) does not match the expected format, in $(refSource). Please check that the definition matches {number} - {type} - {text}."),
        ...
        if self.isStatementOfEquity:
            ...
            if len(sortedList)==0:
                ...
                self.filing.modelXbrl.warning("EFM.6.26.03",
                    _("[rq-2603-No-Matching-Durations] The presentation group ''$(linkroleName)'' is a statement of changes in equity, but it has no duration-type facts matching the instant-type facts shown.  Add duration-type facts that represent the changes from instant to instant, or do not label the presentation group a statement of changes."),
