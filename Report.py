    ...
    def generateBarChart(self):
        ...
        if numYears > 20:
            ...
            self.filing.modelXbrl.error("EFM.6.26.08",
                                   _('[rq-2608-Too-Many-Annual-Return-Facts] In ''$(linkroleName)'', the embedded report created by the fact $(fact) with context $(contextID), there are $(numYears) Annual Return Facts, but there must not be more than 20.'),
        ...
        if embedding.cube.isBarChart:
            ...
            if fig == 'tooManyFacts':
                ...
                self.filing.modelXbrl.warning("EFM.6.26.07",
                                       _('[rq-2607-Bar-Chart-Has-No-Facts] In ''$(linkroleName)'', the embedded report created by the fact $(fact) with context $(contextID), is a bar chart figure that has zero facts. If a bar chart figure is not wanted here, consider removing the text block fact; otherwise, determine why all its facts are being filtered out.'),
